# Thanks to https://github.com/artemyk/ibsgd/ for simplified bin calculation script.
import numpy as np
...
def bin_calc_2dinformation(labelixs, layerdata, binsize):
    # This is even further simplified, where we use np.floor instead of digitize
    def get_h(d):
        H = np.array(np.shape(d)[0])
        for dim in d:
            digitized = np.floor(d / binsize).astype('int')
            p_ts, _ = get_unique_probs( digitized )
            H[dim] = -np.sum(p_ts * np.log(p_ts))
